...
describe('LoanDetails', () => {
	it('renders the page', () => {
		render(<LoanDetails />);
		expect(
			screen.getByRole('heading', { level: 1, name: 'Fill in loan details' }),
		);
		expect(
			screen.getByRole('spinbutton', { name: /Approximate vehicle price/i }),
		);
	...
	it('simulates filling info', async () => {
		...
		await user.selectOptions(
			screen.getByRole('combobox', { name: /Loan Purpose/i }),
			'Vehicle',
		);
		await user.selectOptions(
			screen.getByRole('combobox', { name: /Loan Term/i }),
			'2 years',
		);
		await user.click(screen.getByRole('button', { name: 'Continue' }));
