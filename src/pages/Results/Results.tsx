...
export default function Results() {
	...
	const [recomResults, setRecomResults] = useState<RecommendedResult[] | null>(
		null,
	);
	...
	return (
		<div className={styles.Results}>
			...
			{fetchStatus === 'loading' && (
				<p>Formulating lender recommendations...</p>
			)}
			{fetchStatus === 'error' && (
				<p>
					There was an error fetching the recommendations. Please try again
					later.
				</p>
			)}
			<ul className={styles.list}>
				{recomResults ? (
					recomResults.map((item) => {
						return (
							<li key={item.id}>
								<div className={styles.Card}>
									...
									<div>
										<p>
											<strong>Repayment:</strong> {item.repayment}
										</p>
										<p>
										...
										{item.fees.map((fee) => (
											<p key={fee.id}>
												<strong>Fees:</strong> ${fee.amount} {fee.type} fee
											</p>
										))}
									</div>
								</div>
