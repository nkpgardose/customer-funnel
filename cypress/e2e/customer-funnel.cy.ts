describe('Customer funnel flow', () => {
	...
	describe('User fills out personal details', () => {
		it('navigates and fills out personal detail form', () => {
			...
			cy.findByRole('combobox', { name: /Employment Status/i }).select(
				'Unemployed',
			);
			cy.findByRole('button', { name: /Continue/i }).click();
			cy.findByRole('heading', {
				level: 1,
				name: 'Fill in loan details',
			}).should('exist');
		});
	});
	...
	describe('User fills out personal details and loan details', () => {
		it('navigates and fills out personal detail form', () => {
			...
			cy.findByRole('spinbutton', { name: /Approximate vehicle price/i })
				.clear()
				.type('20000');
			cy.findByRole('spinbutton', { name: /Deposit amount/i })
				.clear()
				.type('2500');
			...
			cy.findByRole('heading', {
				level: 1,
				name: 'List of recommended lenders',
			}).should('exist');
		});
	});
});
